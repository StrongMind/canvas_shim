...
ToDoListPresenter.class_eval do

  ASSIGNMENT_LIMIT = 100
  VISIBLE_LIMIT = 5

  def initialize(view, user, contexts, full_view = false)
    @view = view
    @user = user
    @full_view = full_view
    @contexts = contexts
    ...
    if user
      ...
      if @full_view
        assessment_requests = user.submissions_needing_peer_review(contexts: contexts)
      else 
  ...
  def assignments_needing(type, opts = {})
    if @user
      ...
      scope[:limit] = ::ASSIGNMENT_LIMIT unless @full_view
      @user.send("assignments_needing_#{type}", scope.merge(opts)).map do |assignment|
  ...
  def visible_limit
    return current_count if @full_view
    VISIBLE_LIMIT
  end

  def current_count
    needs_grading.count + needs_moderation.count + needs_submitting.count + needs_reviewing.count
  end
