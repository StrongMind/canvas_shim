Submission.class_eval do
  ...
  after_commit :send_needs_regrading_alert_if_needed
  ...
  def send_needs_regrading_alert_if_needed
    if !self.previous_changes.include?(:grader_id) and self.previous_changes.include?(:submitted_at) and self.grader_id == 1 and self.score == 0
