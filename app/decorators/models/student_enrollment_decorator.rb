StudentEnrollment.class_eval do
  before_commit :strongmind_final_score_recalculation, if: -> { scores.empty? }
  ...
  def missing_assignments_count
    ...
    .eager_load(:assignment).where("assignments.workflow_state NOT IN ('deleted', 'unpublished')").count
  end
