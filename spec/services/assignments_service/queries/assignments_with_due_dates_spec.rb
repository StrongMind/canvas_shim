describe AssignmentsService::Queries::AssignmentsWithDueDates do
  ...
  let!(:content_tag_3) { ContentTag.create(content_type: 'Quizzes::Quiz', assignment: assignment_2, context_module: context_module)}
  ...
  describe '#query' do
    it 'returns assignments' do
      expect(subject.query).to eq [assignment, assignment, assignment_2]
    end

    context 'other type' do
      let!(:content_tag_3) { ContentTag.create(content_type: 'ImaginaryShaw', assignment: assignment_2, context_module: context_module)}

      it 'returns assignments' do
        expect(subject.query).not_to include assignment_2
      end
    end
