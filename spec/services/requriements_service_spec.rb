describe RequirementsService do
  ...
  let(:command_class) { RequirementsService::Commands::ApplyMinimumScoresToUnit }
  ...
  describe '#apply_minimum_scores_to_unit' do
    it 'Calls the command object' do
      expect(command_instance).to receive(:call)
      subject.apply_minimum_scores_to_unit(context_module: context_module)
    end
    ...
    context "stripping overrides" do
      it 'accepts a strip overrides parameter' do
        ...
        subject.apply_minimum_scores_to_unit(context_module: context_module, force: true)  
      end
    end
  end
end